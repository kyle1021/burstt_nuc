...
for fMHz in fMHzs:
    ...
    if (do_eigen):
        keys.extend(['winN2FT', 'winN2FTNull1', 'winN2FTNull2'])
        keys.extend(['winVarNull1', 'winVarNull2'])
        keys.extend(['winSpecNull1', 'winSpecNull2'])
        keys.extend(['winCoeffNull1', 'winCoeffNull2'])
        keys.extend(['winMedCoeffNull1', 'winMedCoeffNull2'])
        ...
        for j in range(nWin):
            ...
            if (auto_align):
                ...
                for ipa, pa in enumerate(pairs):
                    ai, aj = pa
                    corr_arr = correlate(data[ai].filled(), data[aj].filled(), mode='same')
                ...
                savdata['winAlign'].append(med_off)
            ...
            savdata['winNFT'].append(nFTspec)                   # auto-correlation
            ...
            nRun = 1        # only nFTdata
            if (do_eigen):
                nRun = 3    # also do null1 and null2

                Cov, norm = makeCov(nFTdata)
                #n2FTdata = nFTdata / norm.reshape((nInp, 1, nChan))
                n2FTdata = nFTdata * 1.     # try not to change normalization
                W, V = Cov2Eig(Cov)

                n2FTspec = np.ma.median(np.ma.abs(n2FTdata), axis=1)          # shape (nInp, nChan)
                savdata['winN2FT'].append(n2FTspec)                   # auto-correlation, normalized
                savmask['winN2FT'].append(n2FTspec.mask)

                #null0 = np.zeros_like(n2FTdata)   # nulling of V3
                null1 = np.zeros_like(n2FTdata)   # nulling of V3
                null2 = np.zeros_like(n2FTdata)   # nulling of V3+V2
                for ii in range(nChan):
                    ...
                    #null0[:,:,ii] = np.tensordot(sane, n2FTdata[:,:,ii], axes=(1,0))
                    null1[:,:,ii] = np.tensordot(rem1, n2FTdata[:,:,ii], axes=(1,0))
                    null2[:,:,ii] = np.tensordot(rem2, n2FTdata[:,:,ii], axes=(1,0))
                #null0.mask = n2FTdata.mask
                null1.mask = n2FTdata.mask
                null2.mask = n2FTdata.mask
                del n2FTdata
                gc.collect()

                null1spec = np.ma.median(np.ma.abs(null1), axis=1)          # shape (nInp, nChan)
                savdata['winN2FTNull1'].append(null1spec)                   # auto-correlation
                savmask['winN2FTNull1'].append(null1spec.mask)
                null2spec = np.ma.median(np.ma.abs(null2), axis=1)          # shape (nInp, nChan)
                savdata['winN2FTNull2'].append(null2spec)                   # auto-correlation
                savmask['winN2FTNull2'].append(null2spec.mask)
            ...
            for rr in range(nRun):
                ...
                for ai in range(nInp-1):
                    for aj in range(ai+1, nInp):
                        ...
                        if (rr == 0):   # nFTdata (no null, not n2FTdata)
                            xdatai = nFTdata[ai]
                            xdataj = nFTdata[aj]
                        elif (rr == 1): # null1
                            xdatai = null1[ai]
                        ...
                        xc     = xtmp / (np.ma.abs(xdatai) * np.ma.abs(xdataj))
                        coeff[b] = xc
                ...
                specname0  = 'winSpec'
                coeffname0 = 'winCoeff'
                medcoeffname0 = 'winMedCoeff'
                varname0   = 'winVar'
                if (rr == 0):
                    specname = specname0
                    coeffname = coeffname0
                    medcoeffname = medcoeffname0
                    varname = varname0
                    runname = 'raw'
                elif (rr == 1):
                    specname  = specname0 + 'Null1'
                    coeffname = coeffname0 + 'Null1'
                    medcoeffname = medcoeffname0 + 'Null1'
                    varname = varname0 + 'Null1'
                    runname = 'null1'
                elif (rr == 2):
                    specname  = specname0 + 'Null2'
                    coeffname = coeffname0 + 'Null2'
                    medcoeffname = medcoeffname0 + 'Null2'
                    varname = varname0 + 'Null2'
                    runname = 'null2'
                savdata[specname].append(xspec)
                savmask[specname].append(xspec.mask)
                savdata[coeffname].append(coeffspec)
                savmask[coeffname].append(coeffspec.mask)

                med_coeff  = np.ma.median(np.ma.abs(coeffspec), axis=1)
                savdata[medcoeffname].append(med_coeff)

                var = xcorr.var(axis=1) # along nFrame
                var.mask = xcorr.mask.all(axis=1)
                savdata[varname].append(var)   # shape(nBl, nChan)
                savmask[varname].append(var.mask)
                ...
                for pt in range(2):     # amp and pha plots
                    if (pt == 0):
                        png1 = '%s/%s.correlate_amp.%s.png' % (odir, runname, tag)
                        ylab = 'power (dBm)'
                    elif (pt == 1):
                        png1 = '%s/%s.correlate_pha.%s.png' % (odir, runname, tag)
                        ylab = 'phase (rad)'
                ...
                png2 = '%s/%s.correlate_var.%s.png' % (odir, runname, tag)
                ...
                png3 = '%s/%s.correlate_coeff.%s.png' % (odir, runname, tag)
    ...
    if (do_eigen):
        winNFT = np.ma.array(savdata['winN2FT'], mask=savmask['winN2FT'])
        adoneh5(fout, winNFT, 'winN2FT')
        putAttrs(fout, {'note':'variance-normalized auto-correlation spectrum'}, dest='winN2FT')

        winCoeffNull1 = np.ma.array(savdata['winCoeffNull1'], mask=savmask['winCoeffNull1'])
        adoneh5(fout, winCoeffNull1, 'winCoeffNull1')
        putAttrs(fout, {'note':'window correlation coefficients after removing 1 eigenmode',
                        'median':savdata['winMedCoeffNull1']}, dest='winCoeffNull1')
        winCoeffNull2 = np.ma.array(savdata['winCoeffNull2'], mask=savmask['winCoeffNull2'])
        adoneh5(fout, winCoeffNull2, 'winCoeffNull2')
        putAttrs(fout, {'note':'window correlation coefficients after removing 2 eigenmodes',
                        'median':savdata['winMedCoeffNull2']}, dest='winCoeffNull2')

        winSpecNull1 = np.ma.array(savdata['winSpecNull1'], mask=savmask['winSpecNull1'])
        adoneh5(fout, winSpecNull1, 'winSpecNull1')
        winSpecNull2 = np.ma.array(savdata['winSpecNull2'], mask=savmask['winSpecNull2'])
        adoneh5(fout, winSpecNull1, 'winSpecNull2')

        winVarNull1 = np.ma.array(savdata['winVarNull1'], mask=savmask['winVarNull1'])
        adoneh5(fout, winSpecNull1, 'winVarNull1')
        winVarNull2 = np.ma.array(savdata['winVarNull2'], mask=savmask['winVarNull2'])
        adoneh5(fout, winSpecNull2, 'winVarNull2')
