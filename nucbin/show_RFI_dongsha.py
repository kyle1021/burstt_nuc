...
allspec -= 20   # scale back to 20dB gain, in which the bandpass was calibrated
#allspec[:,8] -= 10.   # 540MHz
#allspec[:,9] -= 10.   # 580MHz
...
f1.suptitle('%s--%s RFI @ DONGSHA station, with filters, %s' % (t1str, t2str, dev))
f1.savefig('RFI_DONGSHA_%s-%s_%s.png' % (t1str, t2str, dev))
plt.close(f1)
...
f2.suptitle('%s--%s RFI @ DONGSHA station, with filters, %s' % (t1str, t2str, dev))
f2.savefig('WF_DONGSHA_%s-%s_%s.png' % (t1str, t2str, dev))
plt.close(f2)
