...
sub[0,0].set_ylabel('SEFD*(L/L0)^2 (MJy)')
#sub[0,0].set_xlabel([])
sub[0,0].grid()
sub[1,1].set_xlabel('SEFD*(L/L0)^2 (MJy)')
#sub[1,1].set_yticks([])
sub[1,1].grid()
sub[0,1].remove()
...
for fi in range(nFreq):
    ...
    tsys2d  = getData(fvis, 'winSEFD2D') / 1e6
...
fig.text(0.77, 0.80, 'med(SEFD*(L/L0)^2): %.3f MJy'%tsys_freq2_med)
