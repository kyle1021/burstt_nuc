...
f1.suptitle('%s--%s RFI @ CAFE station, with filters, %s' % (t1str, t2str, dev))
f1.savefig('RFI_CAFE_%s-%s_%s.png' % (t1str, t2str, dev))
plt.close(f1)
...
f2.suptitle('%s--%s RFI @ CAFE station, with filters, %s' % (t1str, t2str, dev))
f2.savefig('WF_CAFE_%s-%s_%s.png' % (t1str, t2str, dev))
plt.close(f2)
