...
nWin    = 48         # num of windows to check
winSep  = 300.        # window separation in seconds
winOff  = 0.        # overall window offset in seconds
verbose = 0
# data directoies
idir    = {'dev1':'data_%.0fMHz'%fMHz, 'dev2':'data_%.0fMHz'%fMHz}    # key = device name (e.g. dev1, dev2, ...); value = directory (here 'dev1/', 'dev2/')
devs    = list(idir.keys())
nAnt    = int(2*len(devs))
...
files = glob('%s/*_%s.h5'%(idir[d0],d0))
files.sort()
nfiles = len(files)
if (nfiles < 1):
...
for i in range(iStart, iEnd):
    ...
    for j in range(nWin):
        ...
        #print('main:', data0.shape, data0.mask.shape)
        #print('main:', data1.shape, data1.mask.shape)
        #data = np.ma.array([data0[:,0], data0[:,1], data1[:,0], data1[:,1]])
        data = np.ma.array(np.zeros((4,winLen), dtype=np.complex64), mask=False, fill_value=0)
        data[0] = data0[:,0]
        data[1] = data0[:,1]
        data[2] = data1[:,0]
        data[3] = data1[:,1]
        #norm = np.array([x.var() for x in data])
        #print('main:', data.shape, data.mask.shape)
        ...
        rfi_mask = []
        for ch in range(4):
            var = FTdata[ch].var(axis=0)    # real numbers
        ...
        for ch in range(4):
            ax = s2[ch]
            ax.plot(chan, 20.*np.log10(np.abs(FTdata[ch]).mean(axis=0)), label='original')
            ax.plot(chan[~rfi_mask], 20.*np.log10(np.abs(FTdata[ch]).mean(axis=0)[~rfi_mask]), label='masked')
            if (ch==0):
                ax.legend()
